  ...
  if (table === "student") {
    ...
    db.query(
      `select school,degree,school_gpa,study_major,start_year,end_year from student_education where student_id = ?;
              select about_me, strengths_qualities, location, school_grade_level, resume, profile_image from
              student_profile_page where student_id=?;  #profile
              select project_name,summary,arr_tools_used,professor,links_website,arr_collaborators_arr from
              student_projects where student_id=?; #projects
              select sr.student_id,
                     ...
                     sr.publish_date,
                     sr.rating_total from student_ratings sr inner join professors p on sr.professor_id = p.professor_id
                     where sr.student_id = ?;
    ...
    db.query(
      `select first_name, last_name, organization_name, email from employers where employer_id=?;
              select listing_id, position_title, location, job_type, experience_years, salary, about_us,
               the_opportunity, task_responsibilities, skillset, benefits
