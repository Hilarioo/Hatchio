...
module.exports = (app) => {
  var insertRoute = function(url, query, jsonKeys) {
    app.post(url, (req, res) => {
      try {
        SQL_CONNECTION.query(query, jsonKeys.map(key => req.body[key]), (err, results) => {
          if (err) {
  ...
  insertRoute('/insert_student_projects',
              ...
              ['Student_ID', 'project_name', 'summary', 'arr_tools_used', 'arr_tools_used' /* TODO I assume this should be professor */, 'links_website', 'arr_collaborators_arr']);
};
